...
def get_keywordclassifier(preload,classifier_type):
  ...
  return {'model': model, 'train_XY':train_XY, 'test_XY':None}
...
def generate_candidates(text):
  ...
  return list(set.union(set(named_entities),set(noun_phrases),set(top_trigrams))), list(set(named_entities))
...
def extract_keywords(text,keyword_classifier,top_k,preload):
  ...
  candidate_keywords, named_entities = generate_candidates(text)
  ...
  # combine named entities and chosen_keywords
  return {'keywords': chosen_keywords, 'named_entities': named_entities}
