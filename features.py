...
def get_namedentities(text):
  ...
  """
  st = StanfordNERTagger('utils/english.conll.4class.caseless.distsim.crf.ser.gz','utils/stanford-ner.jar')   
  ner_tagged = st.tag(remove_punctuation(text).lower().split())     

  named_entities = []
  if len(ner_tagged) > 0:
    ner_len = len(ner_tagged)
    cur_ne = []
    for i in range(ner_len):
      n = ner_tagged[i]

      if (i+1 >= ner_len or ner_tagged[i+1][1] != n[1]) and (cur_ne or n[1] != 'O'):
        cur_ne.append(remove_punctuation(n[0]))
        named_entities.append(' '.join(cur_ne))
        cur_ne = []
      elif n[1] != 'O':
        cur_ne.append(remove_punctuation(n[0]))
  return named_entities
...
def get_binaryfeature(words,selected_words):
  ...
  """
  feature = [1 if x else 0 for x in [(w in selected_words) for w in words]]
  return feature
